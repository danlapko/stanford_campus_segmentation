...
def video_inference_by_triplets(model, vid_path, n_frames_to_process=100):
    vid = cv2.VideoCapture(vid_path)
    length = int(vid.get(cv2.CAP_PROP_FRAME_COUNT))
    h, w = vid.get(cv2.CAP_PROP_FRAME_WIDTH), vid.get(cv2.CAP_PROP_FRAME_HEIGHT)
    fps = vid.get(cv2.CAP_PROP_FPS)
    ...
    vid_writer = cv2.VideoWriter(vid_path.replace("video.mov", "out_.avi"), cv2.VideoWriter_fourcc(*'DIVX'), 30,
                                 tuple(model.input_size[:2] * 2))
...
if __name__ == '__main__':
    ...
    model = StanfordSegModel().load_from_checkpoint(
        "checkpoints_stanford/deeplabv3_effnet-b2/epoch=16-step=8092.ckpt.backup")
    model.to(device)
    model.setup("val")
    ...
    for i_vid, vid_dir in enumerate(model.val_dataset.video_dirs):
        ...
        video_inference_by_triplets(model, vid_path)
